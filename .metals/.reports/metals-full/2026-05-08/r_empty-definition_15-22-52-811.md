error id: file://<WORKSPACE>/src/main/java/br/thomas/App.java:_empty_/task#
file://<WORKSPACE>/src/main/java/br/thomas/App.java
empty definition using pc, found symbol in pc: _empty_/task#
...
public class App {
    ...
    private static Task findTaskById(BufferedReader reader, List<Task> tasks) throws IOException{
        ...
        String input = reader.readLine();
        int id = Integer.parseInt(input);
        return @@task = tasks.stream().filter(
            t -> t.getId() == id
        ).findFirst().get();
    }
}
